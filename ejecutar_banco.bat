...
cd /d "%~dp0"
echo ============================================
echo   Banco de pruebas v1 - Estrategias de contencion
...
echo [4/4] Ejecutando banco de pruebas (prioridad baja, ~6 min)...
start "" /low /wait /b "%VPY%" banco_pruebas\banco.py %*
if errorlevel 1 (echo [ERROR] La ejecucion fallo.) else (
  echo [OK] Ejecucion completada.
  if exist "banco_pruebas\resultados\reporte_banco.html" start "" "banco_pruebas\resultados\reporte_banco.html"
)
pause
endlocal
