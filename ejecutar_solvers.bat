...
cd /d "%~dp0"
echo ============================================
echo   Comparacion de solvers del modelo v3
...
echo [4/4] Comparando solvers (prioridad baja, ~1 min)...
start "" /low /wait /b "%VPY%" banco_pruebas\comparar_solvers.py %*
if errorlevel 1 (echo [ERROR] La ejecucion fallo.) else (
  echo [OK] Ejecucion completada.
  if exist "banco_pruebas\resultados\reporte_solvers.html" start "" "banco_pruebas\resultados\reporte_solvers.html"
)
pause
endlocal
